...
final class PicassoDrawable extends BitmapDrawable {
   ...
   * image.
   */
  static void setBitmap(View target, Context context, Bitmap bitmap,
      Picasso.LoadedFrom loadedFrom, boolean noFade, boolean debugging) {
    Drawable placeholder = null;
    if (target instanceof ImageView) {
    	placeholder = ((ImageView) target).getDrawable();
    } else {
    	placeholder = target.getBackground();
    }
    ...
    if (target instanceof ImageView) {
    	ScaleType scaleType = (ScaleType) target.getTag();
    	if(scaleType!=null){
    		((ImageView) target).setScaleType(scaleType);
    	}
   ...
   * placeholder image.
   */
  static void setPlaceholder(View target, int placeholderResId, Drawable placeholderDrawable) {
	if (target instanceof ImageView) {
		...
		if (placeholderResId != 0) {
			target.setBackgroundResource(placeholderResId);
		} else {
