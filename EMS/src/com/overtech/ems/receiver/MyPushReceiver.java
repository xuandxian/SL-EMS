...
public class MyPushReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		Log.d(TAG, "[MyPushReceiver] onReceive - " + intent.getAction()
		...
		if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
			...
			if (TextUtils.equals(value, "1")) {
				...
				Intent s = new Intent(context, SplashActivity.class);
				s.putExtras(bundle);
				...
				.getAction())) {
			Log.d(TAG,
					"[MyPushReceiver] 用户收到到RICH PUSH CALLBACK: "
			...
			Log.w(TAG, "[MyPushReceiver]" + intent.getAction()
					+ " connected state change to " + connected);
		} else {
			Log.d(TAG,
					"[MyPushReceiver] Unhandled intent - " + intent.getAction());
		}
	}
