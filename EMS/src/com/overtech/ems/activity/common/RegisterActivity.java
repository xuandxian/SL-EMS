...
public class RegisterActivity extends BaseActivity implements OnClickListener {
	...
	private void init(Bundle savedInstanceState) {
		...
		if(savedInstanceState==null){
			...
			if(!mRegisterFragment.isAdded()){
				transaction.add(R.id.fl_register_container,mRegisterFragment, "REGISTER").commit();
			}else{
	...
	public void onBackPressed() {
		...
		if(mCurrentFragment instanceof RegisterFragment){
			...
			FragmentTransaction transaction=manager.beginTransaction();
			transaction
			.hide(mOtherCertificateFragment)
			.show(mWorkCertificateFragment)
			.commit();
			...
			if(mCurrentFragment instanceof RegisterFragment){
				...
				if(mRegisterFragment.isCorrect()){
					...
					if(!mPersonInfoFragment.isAdded()){
						...
						.add(R.id.fl_register_container,mPersonInfoFragment,"PersonInfo")
						.commit();
					}else{
						transaction
						.hide(mRegisterFragment)
				...
				if(mPersonInfoFragment.isAllNotNull()){
					...
					if(!mPersonEduWorkFragment.isAdded()){
						transaction
						.hide(mPersonInfoFragment)
						.add(R.id.fl_register_container,mPersonEduWorkFragment,"PersonEduAndWork")
						.commit();
					}else{
						transaction
						.hide(mPersonInfoFragment)
				...
				if(mPersonEduWorkFragment.isAllNotNull()){
					...
					if(!mIdCardFragment.isAdded()){
						transaction
						.hide(mPersonEduWorkFragment)
						.add(R.id.fl_register_container,mIdCardFragment,"IdCard")
						.commit();
					}else{
						transaction
						.hide(mPersonEduWorkFragment)
				...
				if(mIdCardFragment.isAllNotNull()){
					...
					if(!mWorkCertificateFragment.isAdded()){
						transaction
						.hide(mIdCardFragment)
						.add(R.id.fl_register_container,mWorkCertificateFragment,"WorkCertificate")
						.commit();
					}else{
						transaction
						.hide(mIdCardFragment)
				...
				if(mWorkCertificateFragment.isAllNotNull()){
					...
					if(!mOtherCertificateFragment.isAdded()){
						transaction
						.hide(mWorkCertificateFragment)
						.add(R.id.fl_register_container,mOtherCertificateFragment,"OtherCertificate")
						.commit();
					}else{
						transaction
						.hide(mWorkCertificateFragment)
						.show(mOtherCertificateFragment)
						.commit();
					}
					mNext.setText("下一步");
					...
					new AlertDialog.Builder(mContext)
						...
						.setPositiveButton("确认", new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {
								Utilities.showToast("开始提交", mContext);
							}
